...
TextureManager::~TextureManager()
{
	...
	for (TexInfo_t t: mTextures)
	{
		...
		SDL_LogInfo(SDL_LOG_CATEGORY_VIDEO, "Texture Being Destructed: %s, %p", t.TextureFileName.c_str(), t);
...
int TextureManager::LoadTexture(std::string path)
{
	...
	SDL_LogInfo(SDL_LOG_CATEGORY_VIDEO, "Texture loaded from path %s. TexInfo Mem Location = %p", path.c_str(), &texinfo);
...
{
	SDL_LogInfo(SDL_LOG_CATEGORY_VIDEO, "Textures loading from directory called %p", m_pInstance);
...
const SpriteSequence* TextureManager::GetSequence(std::string sequenceName)
{
	...
	for (SpriteSequence &seq : mSequences)
	{
		...
		{
			SDL_Log("Sequence %s successfully assigned through GetSequence() ", sequenceName.c_str());
			return &seq;
		}
	}
...
const TexInfo_t* TextureManager::GetSprite(std::string sequenceName)
{
	//& for passing through reference!!!
	for (TexInfo_t& seq : mTextures)
	{
		if (seq.TextureFileName == sequenceName)
		{
			SDL_LogInfo(SDL_LOG_CATEGORY_VIDEO,"Sequence %s successfully assigned through GetSprite() ", sequenceName.c_str());
			return &seq;
		}
	}
	SDL_LogError(SDL_LOG_CATEGORY_ERROR,"Sequence %s Not found. Please check spelling.", sequenceName.c_str());
	return NULL;
}